<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Course.aspx.cs" EnableEventValidation="false"
    Inherits="adminPanel_Course" %>
...
<head>
    ...
    <script type="text/jscript">
        ...
        var ddl = null;
        var ddlFaculty = null;
        var ddlDepartment = null;
        var ddlClassDegree = null;
        var facultyName = null;
        var departmentName = null;
        var classDegree = null;
        $(document).ready(function () {
            ddlFaculty = document.getElementById("<%=ddlFaculty.ClientID %>");
            ddlDepartment = document.getElementById("<%=ddlDepartment.ClientID %>");
            ddlClassDegree = document.getElementById("<%=ddlClassDegree.ClientID %>");
            getInformation(1, null);

        });

        function getInformation(type, facultyName, departmentName) {
            if (type == 1) {
                ddl = ddlFaculty;
                PageMethods.getFacultyName(onSuccess);
            } else if (type == 2) {
                ddl = ddlDepartment;
                PageMethods.getDepartmentName(facultyName, onSuccess);
                ddlClassDegree.options.length = 0;
            } else if (type == 3) {
                ddl = ddlClassDegree;
                PageMethods.getClassDegrees(facultyName, departmentName, onSuccess);
                getCourses();
            }
        }
        ...
        function getDepartmentNames() {
            facultyName = ddlFaculty.options[ddlFaculty.selectedIndex].text;
            getInformation(2, facultyName);
        }
        function getClassDegrees() {
            departmentName = ddlDepartment.options[ddlDepartment.selectedIndex].text;
            getInformation(3, facultyName, departmentName);
        ...
        function findMaxValue() {
            ...
            $('#ddlClassDegree option').each(function () {
                var option = this.text;
                var val = parseInt(option, 10);
                if (option === 'All') {
                    val = 0;
                }
        ...
        function getCourses() {
            try {
                classDegree = ddlClassDegree.options[ddlClassDegree.selectedIndex].text;
            }
            catch (err) {
                classDegree = 'All';
            }
            ...
            obj.classDegree = classDegree;

            $.ajax({
                type: "POST",
                url: "Course.aspx/getCourse",
        ...
        function resetTabullet() {
            $("#table").tabullet({
                ...
                action: function (mode, data) {
                    if (mode === 'save') {
                        if (facultyName === 'Choose...') {
                            alert("Lütfen Fakülte seçiniz.");
                            return;
                        } else if (departmentName === 'Choose...') {
                            alert("Lütfen Bölüm seçiniz.");
                            return;
                        }

                        if (!isNaN(parseInt(data.classDegree, 10))) {
                            if (findMaxValue() >= data.classDegree) {
                                add_update_delete_course(1, data.name, null, data.classDegree);
                            } else {
                                alert("Bu bölüm için en fazla sınıf sayısı " + findMaxValue() + " dir.");
                                return;
                            }

                        } else {
                            alert("Lütfen kaçıncı sınıf alanına sadece rakam giriniz.");
                            return;
                        }
                        source.push(data);
                        getCourses();
                    ...
                    if (mode === 'edit') {
                        for (var i = 0; i < source.length; i++) {
                            if (source[i].id == data.id) {
                                ...
                                if (!isNaN(parseInt(data.classDegree, 10))) {
                                    if (findMaxValue() >= data.classDegree) {
                                        add_update_delete_course(2, data.name, data.id, data.classDegree);
                                    } else {
                                        alert("Bu bölüm için en fazla sınıf sayısı " + findMaxValue() + " dir.");
                                        return;
                                    }
                    ...
                    if (mode == 'delete') {
                        for (var i = 0; i < source.length; i++) {
                            if (source[i].id == data) {
                                add_update_delete_course(-1, " ", source[i].id, 0);
                                source.splice(i, 1);
                                break;
                            }
                        }
        ...
        function add_update_delete_course(processType, courseName, courseId, classDegree) {
            ...
            $.ajax({
                type: "POST",
                url: "Course.aspx/add_update_delete_course",
                data: JSON.stringify(obj),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <table class="table table-hover" id="table1">
                                    <thead>
                                        <tr data-tabullet-map="id1">
                                            <th width="80" data-tabullet-map="_index1" data-tabullet-readonly="true">
                                                Faculty :
                                            </th>
                                            <th data-tabullet-map="_index1" data-tabullet-readonly="true">
                                                <div class="modal-body">
                                                    <asp:DropDownList ID="ddlFaculty" runat="server" Width="200px" onchange="getDepartmentNames()">
                                                    </asp:DropDownList>
                                                </div>
                                            </th>
                                            <th width="80" data-tabullet-map="_index1" data-tabullet-readonly="true">
                                            ...
                                            <th data-tabullet-map="_index1" data-tabullet-readonly="true">
                                                <div class="modal-body">
                                                    <asp:DropDownList ID="ddlDepartment" runat="server" Width="200px" onchange="getClassDegrees()">
                                                    </asp:DropDownList>
                                                </div>
                                            </th>
                                            <th width="80" data-tabullet-map="_index1" data-tabullet-readonly="true">
                                            ...
                                            <th data-tabullet-map="_index1" data-tabullet-readonly="true">
                                                <div class="modal-body">
                                                    <asp:DropDownList ID="ddlClassDegree" runat="server" Width="200px" onchange="getCourses()">
                                                    </asp:DropDownList>
                                                </div>
                                            </th>
                                        </tr>
                                ...
                                <table class="table table-hover" id="table">
                                    <thead>
                                        <tr data-tabullet-map="id">
                                            ...
                                            <th data-tabullet-map="classDegree">
                                                Class Degree
                                            </th>
