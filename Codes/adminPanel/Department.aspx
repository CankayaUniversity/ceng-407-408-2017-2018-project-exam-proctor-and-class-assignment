<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Department.aspx.cs" EnableEventValidation="false"
    Inherits="adminPanel_Department" %>
...
<head>
    ...
    <script type="text/jscript">
        ...
        $(document).ready(function () {
            ddl = document.getElementById("<%=ddlFaculty.ClientID %>");
        ...
        function add_update_delete_department(processType, departmentName, yearNumber, departmentId) {

            var IpAdress = '<%= Request.UserHostAddress%>'.toString();
            var facultyName_ = null;
            facultyName_ = ddl.options[ddl.selectedIndex].text;
            var obj = {};
            obj.processType = processType;
            obj.facultyName = facultyName_;
            obj.departmentName = departmentName;
            obj.yearNumber = yearNumber;
            obj.ipAddress = IpAdress;
            obj.departmentId = departmentId;
            $.ajax({
                type: "POST",
                url: "Department.aspx/add_update_delete_department",
        ...
        function getDepartmentNames() {
            ...
            facultyName = ddl.options[ddl.selectedIndex].text;
            var obj = {};
            obj.facultyName = facultyName;

            $.ajax({
                type: "POST",
                url: "Department.aspx/getDepartmenName",
                data: JSON.stringify(obj),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        function resetTabullet() {
            $("#table").tabullet({
                ...
                action: function (mode, data) {
                    if (mode === 'save') {
                        source.push(data);
                        if (!isNaN(parseInt(data.yearNumber, 10))) {
                            add_update_delete_department(1, data.name, data.yearNumber, 0);
                            getDepartmentNames();
                        } else {
                            alert("Lütfen bölüm yıl sayısını rakam giriniz.");
                            getDepartmentNames();
                        }
                    ...
                    if (mode === 'edit') {
                        for (var i = 0; i < source.length; i++) {
                            ...
                            if (source[i].id == data.id) {
                                source[i] = data;
                                if (!isNaN(parseInt(data.yearNumber, 10))) {
                                    add_update_delete_department(2, data.name, data.yearNumber, data.id);
                                } else {
                                    alert("Please enter the total year of department:");
                                    getDepartmentNames();
                                }
                    ...
                    if (mode == 'delete') {
                        for (var i = 0; i < source.length; i++) {
                            if (source[i].id == data) {
                                alert(source[i].id);
                                add_update_delete_department(-1," ",0,source[i].id);
                                source.splice(i, 1);
                                break;
                            }
                        }
                                ...
                                <table class="table table-hover" id="table">
                                    <thead>
                                        <tr data-tabullet-map="id">
                                            ...
                                            <th width="50" data-tabullet-map="yearNumber">
                                                Year Number
                                            </th>
