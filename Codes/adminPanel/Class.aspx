<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Class.aspx.cs" EnableEventValidation="false"
    Inherits="adminPanel_Class" %>
...
<head>
    ...
    <script type="text/jscript">
        ...
        function resetTabullet() {
            $("#table").tabullet({
                ...
                action: function (mode, data) {
                    if (mode === 'save') {
                        source.push(data);
                        add_update_delete_className(1, 0, data.name, data.classCapacity, data.undefined);
                        resetTabullet();
                    }
                    if (mode === 'edit') {
                        for (var i = 0; i < source.length; i++) {
                            ...
                            if (source[i].id == data.id) {
                                source[i] = data;
                                add_update_delete_className(2, data.id, data.name, data.classCapacity, data.undefined);
                                getClassNames();                                
                            }
                        }
                    }
                    if (mode == 'delete') {
                        for (var i = 0; i < source.length; i++) {
                            if (source[i].id == data) {
                                add_update_delete_className(-1, source[i].id, " ", 0, " ");
                                source.splice(i, 1);
                                break;
                            }
                        }
        ...
        function add_update_delete_className(processType, classId, className, classCapacity, classTypeName) {
            ...
            try {
                ...
                obj.classCapacity = classCapacity;
                obj.classTypeName = classTypeName;
                obj.ipAddress = ipAddress;

                $.ajax({
                    type: "POST",
                    url: "Class.aspx/add_update_delete_className",
        ...
        function getItem(selectValue) {

            var x = JSON.parse(items);
            var result = null;
            for (var i in x) {
                if (i == 0) {
                    if (x[i].name == selectValue) {
                        result = "<option selected=\"selected\">" + x[i].name; +"</option>";
                    } else {
                        result = "<option>" + x[i].name; +"</option>";
                    }
                                ...
                                <table class="table table-hover" id="table">
                                    <thead>
                                        <tr data-tabullet-map="id">
                                            ...
                                            <th data-tabullet-map="classCapacity">
                                                Class Capacity
                                            </th>
                                            <th width="200" data-tabullet-map="selectValue" data-tabullet-type="select">
                                                Class Type
                                            </th>
